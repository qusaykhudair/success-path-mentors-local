...
Current correction superseding the fallback described below: the public signup page has NO link or local state transition to the legacy form. `RegistrationForm` renders only method selection. The retained `RegistrationCompletionForm` is not mounted by any public route; its component-only tests do not imply a verified account. It must only be integrated once the backend provides a server-verified signup session and authenticated completion/persistence contract. No callback, local flag or provider selection grants access to it. Pre-registration OTP, OAuth completion and database persistence remain unavailable and unverified. Changed: signup-methods, registration-form, phone-ui tests and this report; no files deleted or schema changes.
