...
type PlaybackFocusTarget = 'play' | 'stop';
...
function extractYouTubeId(
  ...
  try {
    const decoded = decodeURIComponent(cleanedInput);
  ...
  for (const value of values) {
    ...
    try {
      ...
      if (pathTypes.includes(pathParts[0] ?? '')) {
      ...
      // Continue with the next candidate value.
    }
  }
...
function VideoThumbnail({
  ...
  const customThumbnail = normalizeThumbnailSource(
    item.thumbnail
  );
  ...
  if (!currentSource) {
    return (
      <div
        aria-hidden="true"
        className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-primary-950 via-primary-800 to-accent-900"
      >
        <Youtube
    ...
    <Image
      ...
      onError={() => {
        setSourceIndex((current) =>
          Math.min(current + 1, sources.length)
        );
      }}
...
function PlaylistItem({
  ...
  return (
    <button
      ...
      aria-label={`${labels.select} ${index + 1}: ${item.name}`}
      className={cn(
        `
          group
          relative
        ...
        active
          ...
          : 'border-border bg-card hover:-translate-y-0.5 hover:border-accent-300 hover:bg-accent-50/40 hover:shadow-card motion-reduce:hover:translate-y-0'
      )}
    >
      <span className="relative aspect-video overflow-hidden rounded-lg bg-primary-950">
        <VideoThumbnail item={item} sizes="150px" />

        <span
          aria-hidden="true"
          className="absolute inset-0 bg-primary-950/25 transition-colors duration-300 group-hover:bg-primary-950/40"
        />
        ...
        >
          <span className="flex h-9 w-9 items-center justify-center rounded-full bg-white/95 text-primary-900 shadow-lg transition-[transform,background-color,color] duration-300 group-hover:scale-110 group-hover:bg-accent group-hover:text-accent-foreground motion-reduce:transition-none motion-reduce:group-hover:scale-100">
            <Play
              className="h-4 w-4 translate-x-px fill-current rtl:-translate-x-px"
              strokeWidth={0}
            />
          </span>
        </span>
      ...
      <span className="min-w-0">
        <span
          className={cn(
            'block text-caption font-bold uppercase tracking-wider',
            active
              ? 'text-accent-700'
        ...
        <span className="mt-1 block truncate text-small font-bold text-foreground">
          {item.name}
        </span>
        ...
        <span className="mt-2 hidden line-clamp-2 text-caption leading-relaxed text-muted-foreground xl:block">
          {item.quote}
        </span>
      </span>
      ...
      <span
        aria-hidden="true"
        className={cn(
          'absolute inset-y-3 start-0 w-1 rounded-e-full bg-accent transition-opacity duration-300',
...
export function VideoTestimonialsGrid({
  ...
  const stopButtonRef =
    useRef<HTMLButtonElement>(null);
  const focusAfterPlaybackChange =
    useRef<PlaybackFocusTarget | null>(null);

  const selectedItem =
    items[selectedIndex] ?? items[0];

  const selectedVideoId = selectedItem
    ? extractYouTubeId(selectedItem.videoSrc)
    : '';
  ...
  useEffect(() => {
    const target = focusAfterPlaybackChange.current;
    ...
    const frame = window.requestAnimationFrame(() => {
      ...
      focusAfterPlaybackChange.current = null;
    });
  ...
  function startPlayback() {
    if (!hasValidSelectedVideo) {
      return;
    }

    focusAfterPlaybackChange.current = 'stop';
    setIsPlaying(true);
  }

  function stopPlayback() {
    focusAfterPlaybackChange.current = 'play';
    setIsPlaying(false);
  }
  ...
  return (
    <div
      ...
      className="grid items-start gap-6 lg:grid-cols-[minmax(0,1.55fr)_minmax(310px,0.75fr)] xl:gap-8"
    >
      <p
        className="sr-only"
        aria-live="polite"
        aria-atomic="true"
      >
        {labels.nowPlaying}: {selectedItem.name}
      </p>
      ...
      {/* Video playlist */}
      <aside className="overflow-hidden rounded-[1.5rem] border border-border bg-surface-sunken p-4 shadow-card sm:p-5">
        <div className="flex items-center justify-between gap-4 border-b border-border pb-4">
          <div>
            <p className="flex items-center gap-2 text-small font-bold text-foreground">
              <ListVideo
            ...
            <p className="mt-1 text-caption text-muted-foreground">
              {items.length} {labels.youtube}
            </p>
          </div>

          <span
            aria-hidden="true"
            className="flex h-10 w-10 items-center justify-center rounded-full bg-primary text-caption font-bold text-white"
          >
            {String(items.length).padStart(2, '0')}
          </span>
        </div>
        ...
        <div
          ...
          className="mt-4 grid gap-3 sm:grid-cols-2 lg:max-h-[36rem] lg:grid-cols-1 lg:overflow-y-auto lg:pe-1"
        >
