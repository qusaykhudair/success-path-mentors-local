...
export function SignupMethods({ locale }: { locale: AuthLocale }) {
  const ar = locale === 'ar';
  ...
  return <div className="mx-auto w-full max-w-xl">
    ...
    {method && <section className="mt-6 space-y-4" aria-label={ar ? 'التحقق من بيانات التواصل' : 'Verify your contact'}>
      ...
      <Notice>{ar ? 'التحقق قبل إنشاء الحساب غير متاح حاليًا. لم يتم إرسال رمز أو إنشاء حساب. استكمال البيانات يأتي بعد نجاح التحقق.' : 'Verification before account creation is currently unavailable. No code has been sent and no account has been created. Complete your details after successful verification.'}</Notice>
